...
using Probability = Refined<T, IsProbability>;
...
using UnitFloat = Refined<T, IsUnit>;
template <typename T = double>
using UnitDouble = Refined<T, IsUnit>;

// Byte value (0-255)
inline constexpr auto IsByte = InRange(0, 255);
template <typename T = std::int32_t>
using ByteValue = Refined<T, IsByte>;

// Port number (1-65535)
inline constexpr auto IsPort = InRange(1, 65535);
template <typename T = std::int32_t>
using PortNumber = Refined<T, IsPort>;
